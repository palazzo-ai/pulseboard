...
export default function DependencyEditor({
  ...
  const OppListItem = ({ opp, actionType, onAction, actionLabel, actionColor = 'indigo' }) => (
    <div className="flex items-center gap-2 p-2 bg-slate-50 rounded-lg group">
      ...
      <div className="flex-1 min-w-0">
        <div className="text-sm text-slate-900 truncate">{opp.title}</div>
  ...
  return (
    <div className="fixed inset-0 bg-black/15 flex items-center justify-center z-50 p-4" onClick={onClose}>
      <div 
        className="bg-white border border-slate-200 rounded-xl max-w-2xl w-full shadow-xl max-h-[85vh] overflow-hidden flex flex-col"
      ...
      >
        ...
        <div className="flex border-b border-slate-200">
          ...
          >
            ...
            <span className="text-[10px] text-slate-400 ml-2 block">
          ...
          <div>
            ...
            <div className="relative mb-3">
              <input
                ...
                className="w-full px-3 py-2 bg-white border border-slate-200 rounded-lg text-sm text-slate-800 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent"
              />
              {searchQuery && (
                <button
        ...
        <div className="px-4 py-3 border-t border-slate-200 bg-slate-50">
          <div className="text-[10px] text-slate-500 flex items-start gap-3">
            <div className="flex-1">
              <strong className="text-indigo-600">→ Blocks:</strong> Work that cannot start until this opportunity is done
            </div>
            <div className="flex-1">
              <strong className="text-orange-500">← Blocked By:</strong> Work that must finish before this can start
            </div>
          </div>
        </div>
      </div>
...
export function DependencyBadge({ opportunity, onClick }) {
  ...
  return (
    <button
      ...
      className="flex items-center gap-0.5 px-1.5 py-0.5 bg-indigo-50 hover:bg-indigo-100 text-indigo-600 rounded text-[9px] font-medium transition-colors"
