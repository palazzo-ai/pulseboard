...
const TeamMemberForm = ({ member, onSave, onCancel }) => {
  ...
  return (
    <div className="space-y-4">
        ...
        <div className="flex gap-2">
          {COLORS.map(color => (
            <button
              ...
              className={`w-7 h-7 rounded-full border-2 transition-transform ${formData.color === color ? 'border-slate-900 scale-110' : 'border-transparent hover:scale-105'}`}
      ...
      <div>
        <label className="block text-xs text-slate-500 mb-1">Capacity (points/sprint)</label>
        <input
          type="number"
          min="0"
          max="50"
      ...
      <div className="flex justify-end gap-2 pt-2">
        <button type="button" onClick={onCancel} className="px-4 py-2 text-slate-500 hover:text-slate-700 transition-colors">
          Cancel
        </button>
        <button
          type="button"
...
const TeamManagementPanel = ({ isOpen, onClose, teamMembers, onRefresh, showNotification }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black/30 flex items-center justify-center z-50 p-4" onClick={onClose}>
      <div className="bg-white border border-slate-200 rounded-xl max-w-2xl w-full shadow-lg max-h-[85vh] overflow-hidden flex flex-col" onClick={e => e.stopPropagation()}>
        <div className="p-4 border-b border-slate-200 bg-gradient-to-r from-emerald-50 to-teal-50 flex items-center justify-between">
          <div>
            <h2 className="text-lg font-semibold text-slate-900">Team Management</h2>
            <p className="text-xs text-slate-500">{teamMembers.length} active members</p>
          </div>
          <div className="flex items-center gap-2">
            <button
              onClick={() => setIsAdding(true)}
              className="px-3 py-1.5 bg-emerald-600 hover:bg-emerald-700 text-white text-sm rounded-lg transition-colors flex items-center gap-1.5"
            >
          ...
          {teamMembers.length === 0 ? (
            ...
            <div className="space-y-2">
              {teamMembers.map(member => (
                <div key={member.id} className="p-3 border border-slate-200 rounded-lg flex items-center justify-between hover:bg-slate-50 transition-colors">
        ...
        {(editingMember || isAdding) && (
          <div className="fixed inset-0 bg-black/30 flex items-center justify-center z-[60]" onClick={() => { setEditingMember(null); setIsAdding(false); }}>
            <div className="bg-white border border-slate-200 rounded-xl max-w-md w-full p-4 mx-4 shadow-lg" onClick={e => e.stopPropagation()}>
