...
import {
  ...
  DependencyEditor, MultiSelectFilter, AIScoringModal, IssueMapping
} from './components';
          ...
          <form onSubmit={handleSubmit} className="p-4 space-y-4">
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-1.5 mb-2 min-h-[32px] p-2 bg-slate-50 border border-slate-200 rounded-lg">
                ...
                {issueIds.map(id => (
                  <span key={id} className="inline-flex items-center gap-1 px-2 py-0.5 bg-white border border-slate-200 rounded text-xs text-slate-700 font-mono">
                    {id}
                    <button type="button" onClick={() => setIssueIds(prev => prev.filter(i => i !== id))}
          ...
          <div className="bg-white border border-slate-200 rounded-xl max-w-lg w-full shadow-xl" onClick={e => e.stopPropagation()}>
            ...
            <div className="p-4 space-y-4">
              ...
              {selectedOpportunity.issues && selectedOpportunity.issues.length > 0 && (
                <div>
                  <div className="text-slate-500 text-xs uppercase tracking-wide mb-2">Linked Issues</div>
                  <div className="flex flex-wrap gap-1.5">
                    {selectedOpportunity.issues.map(id => (
                      <a key={id} href={`https://linear.app/palazzo-ai/issue/${id}`}
                        target="_blank" rel="noopener noreferrer"
                        className="inline-flex items-center gap-1 px-2 py-0.5 bg-slate-50 border border-slate-200 rounded text-xs text-indigo-600 font-mono hover:bg-indigo-50 hover:border-indigo-200 transition-colors">
                        {id}
